...
// ============================================================================
// DÉTECTION DES ZONES BLANCHES AVANCÉE (avec filtre opérateur optionnel)
// ============================================================================
void CoverageService::getWhiteZonesAdvanced(
    int zone_id,
    std::optional<int> operator_id,
    std::function<void(const Json::Value&, const std::string&)> callback) {
    
    auto client = app().getDbClient();
    
    // Construction dynamique du filtre opérateur
    std::string operatorFilter = "";
    std::string operatorMetadata = "null";
    std::string analysisType = "'global'";
    
    if (operator_id.has_value()) {
        operatorFilter = "AND a.operator_id = $2";
        operatorMetadata = "$2";
        analysisType = "'operator_specific'";
    }
    
    // Requête SQL avec filtre dynamique
    std::string sql = R"(
        WITH target_zone AS (
            SELECT id, geom, name
            FROM zone
        ...
        white_zones_stats AS (
            SELECT 
                ...
                id,
                uncovered,
                ST_Area(zone_geom::geography) / 1000000.0 as total_area_km2,
                ST_Area(uncovered::geography) / 1000000.0 as uncovered_area_km2,
                ROUND(
                    (ST_Area(uncovered::geography) / ST_Area(zone_geom::geography)) * 100,
                    2
                ) as coverage_gap_percent
            FROM white_zones_geom
            WHERE NOT ST_IsEmpty(uncovered)
        )
        SELECT json_build_object(
            ...
            'metadata', json_build_object(
                'zone_id', id,
                'zone_name', name,
                'operator_id', )" + operatorMetadata + R"(,
                'analysis_type', )" + analysisType + R"(,
                'total_area_km2', ROUND(total_area_km2::numeric, 2),
                'uncovered_area_km2', ROUND(uncovered_area_km2::numeric, 2),
                'coverage_gap_percent', coverage_gap_percent,
                'coverage_percent', ROUND((100 - coverage_gap_percent)::numeric, 2)
            ),
            'features', COALESCE(
                json_agg(
                    json_build_object(
                        'type', 'Feature',
                        'geometry', ST_AsGeoJSON((ST_Dump(uncovered)).geom)::json,
                        'properties', json_build_object(
                            'type', 'white_zone',
                            'area_km2', ROUND(
                                (ST_Area((ST_Dump(uncovered)).geom::geography) / 1000000.0)::numeric, 
                                3
                            )
                        )
                    )
                ) FILTER (WHERE uncovered IS NOT NULL),
                '[]'::json
            )
        ) as result
        FROM white_zones_stats
    ...
    // Exécution avec ou sans paramètre operator_id
    if (operator_id.has_value()) {
        client->execSqlAsync(sql,
            [callback](const Result& r) {
                CoverageService::handleWhiteZonesResult(r, callback);
            },
            [callback](const DrogonDbException& e) {
                LOG_ERROR << "CoverageService::getWhiteZonesAdvanced - Database error: " << e.base().what();
                callback(Json::Value(), e.base().what());
            },
            zone_id, 
            operator_id.value()
...
// ============================================================================
// HELPER : Traitement du résultat JSON (évite duplication de code)
// ============================================================================
void CoverageService::handleWhiteZonesResult(
    const Result& r,
    std::function<void(const Json::Value&, const std::string&)> callback) {
    ...
    if (!r.empty() && !r[0]["result"].isNull()) {
        ...
        if (reader->parse(jsonStr.c_str(), jsonStr.c_str() + jsonStr.length(), &result, &errs)) {
            // Vérifier si features est vide (couverture 100%)
            if (result["features"].isArray() && result["features"].empty()) {
                result["metadata"]["message"] = "No white zones found (100% coverage)";
            }
            callback(result, "");
        } else {
            LOG_ERROR << "CoverageService::handleWhiteZonesResult - JSON parsing error: " << errs;
            callback(Json::Value(), "JSON parsing error: " + errs);
        }
    } else {
        // Aucun résultat ou résultat null (100% de couverture)
        Json::Value empty;
        empty["type"] = "FeatureCollection";
        empty["features"] = Json::arrayValue;
        empty["metadata"]["message"] = "No white zones found (100% coverage)";
